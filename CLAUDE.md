...
- The footer now renders on **both** route groups. `app/(tools)/layout.tsx` needs it because the header's "All tools" menu is a Radix dropdown whose links are absent from the HTML until it opens, so the footer is a tool page's only crawlable path to the other nine. `ToolPage` keeps a `min-h-[calc(100svh-4rem)]` floor so the workspace still owns the first viewport.
